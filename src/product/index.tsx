...
const Usage = (props: React.PropsWithChildren<{ step: number, clazz: string, title: string, image: string, href: string }>) => (
  <article className={`productcase_item animated ${props.clazz}`} data-scroll="once toggle(.fadeInUp, .invisible) 0,0">
    <h3><span>{("00" + props.step).slice(-2)}</span>{props.title}</h3>
...
const Feature = (props: React.PropsWithChildren<{ clazz: string, title: string, image1: string, image2: string, href: string }>) => (
  <article className={`productfunction_item animated ${props.clazz}`} data-scroll="once toggle(.fadeInUp, .invisible) 0,0">
    <div className="product_screenimg">
      <div className="screen_img"><img src={props.image1} alt="" /></div>
      <div className="screen_img"><img src={props.image2} alt="" /></div>
    </div>
  ...
  <main>
    ...
    <section id="product_case">
      ...
      <div className="productcase_index">
        <Usage step={1} clazz="" title="野良チャットの置き換え" image="/assets/images/illust/14.png" href="/product/usage-example/#usage01">
            自然発生的に用いられるようになったコンシューマ向けツールをIT統制の観点より組織公式なツールに置き換えるとき。野良チャット撲滅。
        </Usage>
        <Usage step={2} clazz="deley02" title="ちいさな業務のシステム化" image="/assets/images/illust/04.png" href="/product/usage-example/#usage02">
          紙やスプレッドシートの共有で行っているような雑多でちいさな業務をシステム化するとき。使いながら業務をシステムに乗せてDX実現。
        </Usage>
        <Usage step={3} clazz="deley03" title="組織内外を縦断するコミュニケーション" image="/assets/images/illust/08.png" href="/product/usage-example/#usage03">
